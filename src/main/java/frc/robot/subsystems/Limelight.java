...
public class Limelight extends SubsystemBase {
    ...
    /**
     * Override the 3D position of the Limelight relative to the center of the
     * robot.
     * Use to provide accurate 3D geometry for Odometry and MegaTag without relying
     * on the Limelight Web UI configuration.
     * 
     * @param pose Array of 6 doubles: [Forward, Side, Up, Roll, Pitch, Yaw].
    ...
    private static Pose3d toPose3D(double[] vector) {
        // if vector is valid (has 6 numbers in it) go ahead and record data in
        // structure
        if (vector.length < 6)
            return new Pose3d();
        else
